class FinancialHistoryDataController < ApplicationController
  ...
  def index
    ...
    @chart = LazyHighCharts::HighChart.new('graph') do |f|
      ...
      f.series(:name => "DIA (SPDR 1x DJIA ETF)", :yAxis => 1, :color => '#777', :dashStyle => 'ShortDot', :lineWidth => 2, :marker => {:symbol => 'triangle-down', }, :data => FinancialHistoryData.prepare_data_for_chart('dia_last'))
      f.series(:name => "Investor Sentiment", :yAxis => 0, :color => '#3487FF', :dashStyle => 'ShortDot', :lineWidth => 2, :marker => {:symbol => 'triangle-down'}, :data => FinancialHistoryData.prepare_investor_data_for_chart)
      f.series(:name => "Media Sentiment", :yAxis => 0, :color => '#ff1a29', :dashStyle => 'ShortDot', :lineWidth => 2, :marker => {:symbol => 'triangle-down'}, :data => FinancialHistoryData.prepare_media_data_for_chart)
